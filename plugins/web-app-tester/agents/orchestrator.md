...
**Determine `BYPASS_PRODUCTION_CHECK`**:

- If the environment variable `ENVIRONMENT` is set to `production` (case-insensitive) → `BYPASS_PRODUCTION_CHECK=false`
- Otherwise, or if `ENVIRONMENT` is not set → `BYPASS_PRODUCTION_CHECK=true`

The production URL check is **off by default**. To enable it, an operator sets `ENVIRONMENT=production` via `with-envs` in the Xianix Agent `rules.json`.

Store: `ENTRY_TYPE`, `ENTRY_ID`, `BYPASS_PRODUCTION_CHECK`. These are passed through to every phase.
...
Read and follow `skills/gather-test-context/SKILL.md`, passing in `BYPASS_PRODUCTION_CHECK`.

It produces the variables `TEST_URL`, `PRODUCTION_WARNING`, `BYPASS_PRODUCTION_CHECK`, `TEST_PLAN`, and (for `wi` entry on Azure DevOps) `LINKED_PR_ID`. If a testable URL cannot be found, that skill posts a comment and stops the run — do not proceed to Phase 2 in that case.
