...
| `TEST_PLAN` | Flat numbered list of test cases to execute, either found in the entry content or auto-generated from context |
...
If `IS_PRODUCTION=true`, the run is in production mode — data-modifying test cases will be skipped in the execution phase. No action is required here; this is enforced per test case in `skills/run-playwright-session/SKILL.md`.

If `IS_PRODUCTION=false` (default), all test cases run without restriction.
...
When this skill finishes successfully, hand off to `skills/run-playwright-session/SKILL.md` with `TEST_URL`, `IS_PRODUCTION`, `TEST_PLAN`, `ENTRY_TITLE`, `PLAN_SOURCE`, `PLATFORM`, `ENTRY_TYPE`, `ENTRY_ID`, and (if applicable) `LINKED_PR_ID` in scope.
