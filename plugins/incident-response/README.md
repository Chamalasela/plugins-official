...
> An AI **first-responder** for live incidents. Correlates the incident window with recent deployments, surfaces anomalies from logs and metrics, suggests immediate mitigations, and drafts a post-mortem timeline — so your team can focus on resolving the incident, not reconstructing it.
...
1. **Fetch incident** — ADO REST API (work item), GitHub issue, or pasted text. Extracts severity, start time, affected service(s), and any description the team has already written.
...
- **Azure DevOps**: `AZURE_DEVOPS_TOKEN` PAT with `Work Items (Read & Write)` + `Build (Read)` + `Release (Read)` scopes
- **GitHub**: `gh` CLI installed and authenticated (`gh auth login`) — or `GITHUB_TOKEN` env var
- **Azure Monitor logs**: `az` CLI installed ([install guide](https://learn.microsoft.com/en-us/cli/azure/install-azure-cli)) + service principal with `Log Analytics Reader` role, **or** `az login` for local use
- **Metrics** *(optional)*: metrics JSON snapshot via `METRICS_SOURCE` env var
...
See [docs/platform-config.md](docs/platform-config.md) for full credential setup and [docs/incident-sources.md](docs/incident-sources.md) for how to configure log and metrics sources.
...
| `METRICS_SOURCE` | All | No | Path to a metrics JSON snapshot file (`az monitor metrics list` output) |
| `LOG_SOURCE` | All | No | Path to a log JSON snapshot file (`az monitor log-analytics query` output); if unset, the log-analyzer falls back to `METRICS_SOURCE` when the filename contains `logs` |
...
The plugin posts one comment per finding, in this order, preserving the original incident description:

1. **🚨 Incident Summary** — severity, affected services, blast radius window, status signal (`investigating` / `resolved` / `needs-data`)
2. **🚀 Deployment Correlation** — deployments in the blast radius window with cause likelihood, service overlap, and rollback deployment IDs
3. **📊 Logs & Metrics Analysis** — error spike timestamps, exception types, latency percentile anomalies, correlated log patterns
4. **🛠️ Mitigation Suggestions** — ordered action list: immediate steps, rollback candidates (with pipeline/release IDs), config change hints
5. **📝 Post-Mortem Draft** — chronological timeline, contributing factors, action items as discussion prompts for the team

Sections with no real findings are skipped. The post-mortem draft is intentionally rough — it is a **starting point for the team**, not a finished document.

### Work Item Creation (Azure DevOps)

When `ROOT_CAUSE_CONFIDENCE: high` is set by the mitigation advisor **and** the platform is Azure DevOps, the agent automatically creates a linked **Bug** work item for the root cause investigation:

- Work item type: `Bug`
- Title prefixed with `[Root Cause Investigation] #{incident_id}:`
- `ReproSteps` populated from the root cause summary and log/metrics findings
- Tagged `incident-response; ai-generated; {incident_id}`
- Linked to the parent incident via `System.LinkTypes.Hierarchy-Reverse`
- New bug ID appended to the 🚨 Incident Summary comment

See [providers/azure-devops.md](providers/azure-devops.md) for the exact API call shape and [test-data/mock-workitem-bug.json](test-data/mock-workitem-bug.json) for a reference response.
...
The Incident Response Agent is **tag-driven**. It runs when the `ai-dlc/incident/respond` label (GitHub) or tag (Azure DevOps) is present and one of the following happens:
...
|----------|-------------|
| [docs/platform-config.md](docs/platform-config.md) | Azure DevOps PAT scopes, GitHub token setup, service principal creation, CI env vars |
| [docs/incident-sources.md](docs/incident-sources.md) | Azure Monitor log queries, KQL patterns, metrics snapshot export, workspace ID lookup |
| [providers/azure-devops.md](providers/azure-devops.md) | ADO-specific work item fetching, comment posting, and bug/task creation |
| [providers/github.md](providers/github.md) | GitHub Issues and Actions API calls |
| [providers/generic.md](providers/generic.md) | Plain text / unknown platform — file-based output |
| [styles/response.md](styles/response.md) | Tone guide — calm, factual, action-oriented |
| [styles/response-template.md](styles/response-template.md) | Compiled output template |
| [test-data/mock-workitem-bug.json](test-data/mock-workitem-bug.json) | Reference ADO API response for `POST /$Bug` — root cause investigation bug for INC-4821 |
| [test-data/README.md](test-data/README.md) | How to run all analysts against mock data, including work item creation testing |
